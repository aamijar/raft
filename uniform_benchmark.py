...
# A = load_npz("uniform_datasets/uniform_nnz19979746_M100000.npz")
# A = load_npz("uniform_datasets/uniform_nnz79601386_M100000.npz")
# A = load_npz("uniform_datasets/uniform_nnz313388328_M100000.npz")
A = load_npz("uniform_datasets/uniform_nnz1178172851_M100000.npz")
print(A.dtype)
...
start = time.time()
eigenvalues_raft, eigenvectors, eig_iters = eig_lanczos(cuX, k, 42, dtype=np.float32, maxiter=10000, tol=1e-5, conv_n_iters=4, conv_eps=0.001, restartiter=ncv, v0=v0_dev, handle=None)
...
start = time.time()
eigenvalues_cupy, eigenvectors = cueigsh(cuX, k=k, which="SA", maxiter=10000, tol=1e-5, v0=v0_dev)
...
if (X is not None):
    try:
        start = time.time()
        eigenvalues_sklearn, eigenvectors = eigsh((X), k=k, which="SA", maxiter=10000, tol=1e-5)
